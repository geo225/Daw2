  ...
  <!-- Codigo PhP -->
			...
			//Se agregan las librerias para el uso de la encuesta
				...
				if (isset($_POST['enviarFormulario'])){
					...
					$mensajeError["errorCodDepartamento"]=comprobarTexto($_POST['codDepartamento'],3,3);
					...
					$consulta = "SELECT CodDepartamento from Departamento  "; 
					$consulta = $conn->prepare($consulta);
					$consulta->execute();
					while ($resultado = $consulta->fetch(PDO::FETCH_OBJ)) {
					$CodDuplicado= $resultado->CodDepartamento;
					if (strtoupper($_POST['codDepartamento']) == $CodDuplicado){
		...
		//Comprobación de que los errores estan vacios para saber que no ha ocurrido ningun error					
				...
				if (isset($_POST['enviarFormulario']) && $noError==true){
					...
					$codDepartamento=strtoupper(htmlspecialchars(strip_tags(trim($_POST['codDepartamento']))));
					...
					try{
						...
						$sentencia->execute();
						$conn->commit();
						echo "Departamentos agregado con exito";
			...
			?>
						<div class="w3-container" id="contact" style="margin-top:75px;">
							<h1 class="w3-large w3-text-red"><b>Alta</b></h1>
							<hr style="width:50px;border:5px solid red" class="w3-round">
							<form id="formulario" name="FormularioRegistro" action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method="post">
								<div class="w3-section">
									<label class="w3-col">Codigo Departamento<span class="w3-text-red w3-margin-left"><?php echo $mensajeError['errorCodDepartamento'];echo $mensajeError['errorDuplicacion']; ?></span></label>
									<input class="w3-input w3-border w3-half" type="text" name="codDepartamento" required value="<?php if(isset($_POST['codDepartamento'])){ echo $_POST['codDepartamento'];} ?>" >
								...
								<br/><button name="enviarFormulario" type="submit" class="w3-button w3-quarter w3-red w3-margin-bottom w3-margin-left" value="Añadir">Agregar</button><br/>
							</form>
						</div>
			<?php	}
				?>	
