...
<?php
    if (isset($_POST['enviarFormulario'])){
		if ($_POST['enviarFormulario']=="Si"){
			try { 
				...
				$consulta=$conn->prepare("DELETE FROM Departamento where CodDepartamento=:codigo;");
	...
	<div class="w3-container" id="contact" style="margin-top:75px;">
							...
							<form id="formulario" name="FormularioRegistro" action="<?PHP echo $_SERVER['PHP_SELF'].'?CodDepartamento='.$_GET['CodDepartamento'].'&DescDepartamento='.$_GET['DescDepartamento']; ?>" method="post">
								<div class="w3-section">
									...
									<input class="w3-input w3-border w3-half w3-margin-bottom" style="margin-right:300px;"  type="text" name="descDepartamento"  readonly value="<?php echo $_GET['DescDepartamento'] ?>" >									
								</div>      
								<button name="enviarFormulario" type="submit" class="w3-button w3-quarter w3-red w3-margin-bottom w3-margin-top w3-margin-left" value="Si">SI</button>
								<button name="enviarFormulario" type="submit" class="w3-button w3-quarter w3-red w3-margin-bottom w3-margin-top w3-margin-left" value="No">No</button>
							</form>
						</div>
	<?php
	}?>
