...
<?php
	...
	if (isset($_POST['enviarLogin']) && $noError==true){
		...
		try{
			...
			$consulta = "UPDATE `Usuarios` SET `Password` = :pass WHERE `Usuarios`.`Usuario` = :user ;"; 
...
//Formulario que se muestra si no se ha enviado el formulario o ha tenido errores
	...
	<!-- Formulario -->	
		...
		<form id="formulario" name="FormularioLogin" action="<?PHP echo $_SERVER['PHP_SELF']; ?>" method="post">
            ...
            <label>ID Ususario</label>
            <input class="w3-input w3-border" type="text" name="usuario" id="usuario"value="<?php echo $_SESSION['usuario']; ?>" readonly >
            <label>Nueva Contraseña <span class="w3-text-red w3-margin-left"><?php echo $mensajeError['errorPassword'];?></span></label>
            <input class="w3-input w3-border w3-margin-bottom" type="password" name="password" id="password" >
            <label>Repita Contraseña <span class="w3-text-red w3-margin-left"><?php echo $mensajeError['errorDuplicado'];?></span></label>
            <input class="w3-input w3-border w3-margin-bottom" type="password" name="password2" id="password2" >
            <button name="enviarLogin" type="submit" class="w3-button w3-red w3-margin-bottom " value="login">Cambiar</button><br/>
			</div>      
        </form>
    </div>
	<?php }
